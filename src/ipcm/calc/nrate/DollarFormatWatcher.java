...
class DollarFormatWatcher implements TextWatcher {
	...
	private String textAdded = "\\d{1,3}\\x2E\\d{3}";
	private String textRemoved = "\\d{1,3}\\x2E\\d{1}";
	 ...
	 */
	public DollarFormatWatcher(EditText e, EditText fertPrice, EditText nPercent, EditText cornPrice, TextView a, TextView b, ThreeStateToggle yP, TwoStateToggle cP, TextView r, TextView rR)
	{
		et = e;
	...
	private double calcNPrice()
	{
		...
		try
		{
			...
			fertPrice = Double.parseDouble(fertPriceET.getText().toString().substring(0));
		}
		...
		try
		{
			if(nPercentET.getText().toString().length() > 0)
				nPercent = Double.parseDouble(nPercentET.getText().toString().substring(0));
		}
		catch(NumberFormatException e)
		{
			nPercent = 0;
	...
	private double calcRatio()
	{
		...
		try
		{
			if(!cornPriceET.getText().toString().equals(""))
				cP = Double.parseDouble(cornPriceET.getText().toString());
		}
		catch(NumberFormatException e)
		{
			cP = 0;
